...
data "aws_subnet" "main" {
  count = length(data.aws_subnets.asg.ids)

  id = data.aws_subnets.asg.ids[count.index]
}
...
resource "aws_security_group" "ec2" {
  name        = "${var.org_code}-${var.project_code}-${var.asg_name}-asg-sg-${var.env_code}"
  vpc_id      = data.aws_vpc.main.id
  description = "Default security group for the ${local.asg_name} autoscaling group."
