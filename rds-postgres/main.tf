locals {
  ...
  db_name   = "${local.org}-${var.project}-${var.db_name}-${local.db_engine}-rds-${var.environment}"
  db_engine = "psql"
}
...
resource "aws_db_subnet_group" "main" {
  name        = "${local.org}-${var.project}-${var.db_name}-db-sn-grp-${var.environment}"
  description = "Subnet group for ${local.db_name} rds database."
  subnet_ids  = data.aws_subnets.main.ids
}

resource "aws_db_parameter_group" "main" {
  name        = "${local.org}-${var.project}-${var.db_name}-db-param-grp-${var.environment}"
...
resource "aws_iam_role" "enhanced_monitoring" {
  name        = "${local.org}-${var.project}-${var.db_name}-enhanced-monitoring-role-${var.environment}"
...
resource "aws_cloudwatch_log_group" "standby" {
  ...
  name              = "/aws/rds/instance/${local.org}-${var.project}-${var.db_name}-${local.db_engine}-rds-standby${count.index}-${var.environment}/postgresql"
  retention_in_days = 7
}
...
resource "aws_db_instance" "standby" {
  ...
  identifier          = "${local.org}-${var.project}-${var.db_name}-${local.db_engine}-rds-standby${count.index}-${var.environment}"
